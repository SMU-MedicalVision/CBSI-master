...
from Networks.UNet_DDPM import Unet_class
from Networks.DDPM_trainer import GaussianDiffusion, DDPM_Trainer

from Dataset.dataset import CA_Dataset_harmonize_2D
from Nii_utils.utils import NiiDataRead, NiiDataWrite, harmonize_mr, compute_mae
...
def main():
    ...
    if args.continue_train or args.pretrain:
        for i in range(args.continue_epoch + 1):
            scheduler_lr.step()
        trainer.load("./trained_models/DDPM_before0714/pred_noise_simple_unet_Improved_in_relu_32_l1_t1t2_condition_act_none/bs6_epoch200_gae1_seed42/epoch200.pth")

    #  ======================Dataset=====================
    root_dir = './Dataset/glioma_data/Train_val'
    test_dir = './Dataset/glioma_data/Test'
    wtest_dir = './Dataset/glioma_data/99cases'
    ...
    t1, spacing, origin, direction = NiiDataRead(
        './Dataset/glioma_data/99cases/flair1_after/NAME_0001_1002603464/T1.nii.gz')
    t2f, _, _, _ = NiiDataRead('./Dataset/glioma_data/99cases/flair1_after/NAME_0001_1002603464/T2F.nii.gz')
    t1c, _, _, _ = NiiDataRead('./Dataset/glioma_data/99cases/flair1_after/NAME_0001_1002603464/T1C.nii.gz')
    ROI, _, _, _ = NiiDataRead('./Dataset/glioma_data/99cases/flair1_after/NAME_0001_1002603464/ROI.nii.gz',
                               as_type=np.uint8)
    Brain, _, _, _ = NiiDataRead(
        './Dataset/glioma_data/99cases/flair1_after/NAME_0001_1002603464/Brain_mask.nii.gz', as_type=np.uint8)
    ...
    for epoch in range(args.epoch):
        ...
        if (epoch + 1) % (args.epoch // 10) == 0 or epoch == 2:
            ...
            NiiDataWrite(os.path.join(save_dir, 'predictions', f'epoch{epoch}_{MAE}.nii.gz'), syn_pred, spacing, origin,
                         direction)
