...
import cv2
import numpy as np
from Nii_utils.utils import NiiDataRead, circle, img_dilate_3D, harmonize_mr
import pandas as pd
...
if __name__ == '__main__':
    data = Dataset_val(data_root=r'J:\Provincial_TCM_data\NPC_Multimodal_data\MR-CT-nii-20210509_final_for_Fi-GAN',
                         txt_path=r'E:\my_code\MICCAI_FiGAN\NPC_T2_labeled\pre_process/val_set.txt')
    dataloader = DataLoader(dataset=data, batch_size=2, shuffle=True)
    for i, (T2, CT, seg, mask) in enumerate(dataloader):
        T2 = T2[0, 0].numpy()
        CT = CT[0, 0].numpy()
        seg = seg[0].numpy()
        mask = mask[0, 0].numpy()
